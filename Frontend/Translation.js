window.addEventListener('DOMContentLoaded', async () => {
    try {
      const url = 'http://localhost:5000/getCourses';
...
function getFormValue(event) {
    ...
    const data = {course: `${selectedValueCourse}`, score: `${score}`, fieldactivity: `${selectedValueFieldActivity}`};
    putInfo(data);
}
...
const putInfo = async (data) => {
    try {
      const url = `http://localhost:5000/changecourses`;
      const response = await fetch(url, {
        method: 'POST', 
        headers: {
